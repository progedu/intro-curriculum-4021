        ...
        .end((err, res) => {
          ...
          promiseDeleted.then(() => {
            const p1 = Comment.findAll({
              ...
              assert.equal(comments.length, 0); // 配列空判定
            });
            const p2 = Availability.findAll({
              where: { scheduleId: scheduleId }
            }).then((availabilities) => {
              assert.equal(availabilities.length, 0);　// 配列空判定
            });
            const p3 = Candidate.findAll({
              where: { scheduleId: scheduleId }
            }).then((candidates) => {
              assert.equal(candidates.length, 0);　// 配列空判定
            });
            const p4 = Schedule.findByPk(scheduleId).then((schedule) => {
              assert.equal(!schedule, true);　// object null判定
            });
