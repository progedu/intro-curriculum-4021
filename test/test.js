    ...
    User.upsert({ userId: 0, username: 'testuser' }).then(() => {
      request(app)
        ...
        .end((err, res) => {
          ...
          // 「予定に関連する全ての情報が削除できる」というテスト
          //なお findAll 関数は、戻ってきた Promise オブジェクトの then に渡す関数の引数として、 データモデルのオブジェクトの配列を渡します。 findByPk 関数は、then に渡す関数の引数として、 データモデルのオブジェクトを渡し、存在しない場合は null という値となります。
          promiseDeleted.then(() => {
